...
.PHONY: all bench runner pure traverse check fisher-yates fisher-yates-with-array fisher_yates_with_array merge-shuffle shuffle-bench clean

all: bench pure traverse check fisher-yates fisher-yates-with-array merge-shuffle
...
$(BENCH_OBJ): $(SRC_DIR)/bench.c include/create.h include/single_pointer.h include/fast_and_slow.h include/perf_counter.h include/runner_args.h include/list.h | $(BUILD_DIR)
	$(CC) $(CFLAGS) -c $< -o $@

$(PURE_OBJ): $(SRC_DIR)/pure.c include/create.h include/single_pointer.h include/fast_and_slow.h include/runner_args.h include/list.h | $(BUILD_DIR)
	$(CC) $(CFLAGS) -g -c $< -o $@
...
$(BENCH_BIN): $(BENCH_OBJ) $(CREATE_OBJ) $(SINGLE_OBJ) $(FAST_SLOW_OBJ) $(PERF_COUNTER_OBJ) $(RUNNER_ARGS_OBJ) | $(BIN_DIR)
	$(CC) $^ -o $@

$(PURE_BIN): $(PURE_OBJ) $(CREATE_OBJ) $(SINGLE_OBJ) $(FAST_SLOW_OBJ) $(RUNNER_ARGS_OBJ) | $(BIN_DIR)
	$(CC) $^ -o $@
...
clean:
	rm -rf $(BUILD_DIR) $(BIN_DIR)/create $(BENCH_BIN) $(BIN_DIR)/runner $(PURE_BIN) $(TRAVERSE_BIN) $(CHECK_BIN) $(FISHER_YATES_BIN) $(FISHER_YATES_WITH_ARRAY_BIN) $(MERGE_SHUFFLE_BIN) $(LEGACY_SHUFFLE_BENCH_BIN)
